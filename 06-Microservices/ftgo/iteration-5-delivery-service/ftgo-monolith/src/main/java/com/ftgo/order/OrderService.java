...
public class OrderService {
    ...
    @Autowired
    private KitchenServiceClient kitchenService;
    ...
    // ========================================================================
    // ITERATION 5: Delivery is now an external service with Circuit Breaker!
    //
    // Extracted so far:
    //   - Restaurant → REST (iteration 1)
    //   - Notification → Kafka async (iteration 2)
    //   - Accounting → REST + Circuit Breaker (iteration 3)
    //   - Kitchen → REST + Kafka bidirectional (iteration 4)
    //   - Delivery → REST + Kafka bidirectional (iteration 5) ← NEW
    //
    // Delivery-service is the second BIDIRECTIONAL extraction:
    //   - Monolith → Delivery: REST (create delivery)
    //   - Delivery → Monolith: Kafka (status change events update Order)
    //   - Delivery → Kitchen: REST (cross-service check for pickup readiness)
    //
    // The monolith now contains ONLY the Order module!
    // ========================================================================
    @Transactional
    public OrderResponse createOrder(CreateOrderRequest request) {
        ...
        // Step 6: Create kitchen ticket — calls Kitchen Service via REST + Circuit Breaker
        // If kitchen-service is down → circuit breaker trips → fallback rejects order
        ...
        // Step 7: Create delivery — calls Delivery Service via REST + Circuit Breaker
        // If delivery-service is down → circuit breaker trips → fallback rejects order
        deliveryServiceClient.createDelivery(order.getId(), restaurant.getAddress(), request.deliveryAddress());

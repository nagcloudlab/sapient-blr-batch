...
public class AccountingServiceClient {
    ...
    @CircuitBreaker(name = "accountingService", fallbackMethod = "refundPaymentFallback")
    public void refundPayment(Long orderId) {
        log.info(">>> Calling accounting-service to refund payment for order #{}", orderId);
        restTemplate.postForObject(
                "http://accounting-service/api/payments/refund/" + orderId,
                null,
                PaymentResponse.class);
    }

    public void refundPaymentFallback(Long orderId, Throwable t) {
        log.error(">>> CIRCUIT BREAKER: Cannot refund payment for order #{}. Reason: {}", orderId, t.getMessage());
        throw new RuntimeException("Accounting service unavailable — cannot refund payment for order: " + orderId);
    }

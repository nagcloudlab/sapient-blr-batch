...
router.get('/:restaurantId', async (req, res) => {
  try {
    ...
    // BUG: Wrong field name in query - using 'restaurantId' instead of 'id'
    // Expected: Query should use { id: req.params.restaurantId } to match the data schema
...
router.put('/:restaurantId/items/:itemName', async (req, res) => {
  try {
    ...
    // BUG: Uses $push instead of $set to update the price - this adds to array instead of updating
    // Expected: Use $set with positional operator to update matching menu item's price
    const result = await db.collection('restaurants').updateOne(
      { id: req.params.restaurantId, 'menu.item': req.params.itemName },
      { $push: { 'menu.$.price': price } }
    );
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });

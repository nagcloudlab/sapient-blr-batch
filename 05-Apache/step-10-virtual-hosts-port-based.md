...
```bash
echo "Listen 9192" | sudo tee -a /etc/apache2/ports.conf
sudo mkdir -p /var/www/foodapi
echo '<h1>FoodExpress API v2</h1><p>Port 9192</p>' | sudo tee /var/www/foodapi/index.html
```
...
curl http://localhost:9192      # Port 9192
```
...
sudo sed -i '/Listen 9192/d' /etc/apache2/ports.conf
sudo systemctl reload apache2
```
